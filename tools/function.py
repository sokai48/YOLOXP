...
from yolox.evaluators.general import non_max_suppression,check_img_size,scale_coords,xyxy2xywh,xywh2xyxy,box_iou,coco80_to_coco91_class,plot_images,ap_per_class,output_to_target
...
def show_seg_result(img, result, index, epoch, save_dir=None, is_ll=False,palette=None,is_demo=False,is_gt=False):
    ...
    # color_seg = cv2.resize(color_seg, (1242,375), interpolation=cv2.INTER_LINEAR)
    
    # print(color_seg.shape)
    # print("---------")
    # print(img.shape)
    #修改成單純seg 
    # img[color_mask != 0] = color_seg[color_mask != 0]* 1
...
def validate(epoch, config, val_loader , model, logger=None, device='cpu', rank=-1):
    """
    validata
    ...
    save_dir = "/home/lab602.10977014_0n1/.pipeline/10977014/YOLOXP/YOLOX_outputs/"+ os.path.sep + 'visualization'
    ...
    imgsz = (576, 1024)
    batch_size = config.batch_size * config.num_machines
    test_batch_size = config.batch_size * config.num_machines
    training = False
    ...
    seen =  0 
    confusion_matrix = ConfusionMatrix(4) #detector confusion matrix
    ...
    for batch_i, (imgs, _, info_imgs, ids, _) in tqdm(enumerate(val_loader), total=len(val_loader)):
    # for cur_iter, (imgs, _, info_imgs, ids, _) in enumerate(progress_bar(val_loader)
